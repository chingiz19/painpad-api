async function getUserFeed(firstPersonId, { userId, topicId, subTopicId, countryId, postId, lastDate, count = 20}) {
    ...
    SELECT 
        ...
        CAST(COALESCE(solutions.count, 0) AS INT) AS "solutionCnt"
    FROM posts
    ...
    LEFT JOIN(
        SELECT
            ...
            COUNT(id) AS count
        FROM solutions
        GROUP BY post_id) AS solutions ON solutions.post_id = posts.id
    ${whereStr}
...
async function getSolutions(firstPersonId, {userId, postId}) {
    let whereStr = '';
    let whereArr = [];
    let counter = 1;
    let params = [];
    ...
    if (userId) {
        whereArr.push(`s.user_id=$${counter}`);
        params.push(userId);
        counter++;
    }

    if (postId) {
        whereArr.push(`s.post_id=$${counter}`);
        params.push(postId);
        counter++;
    }
    ...
    let query = `
    SELECT S.id, S.logo, S.name, S.website, S.description, extract(epoch from S.created) * 1000 AS created,
            CAST(COALESCE(likes.count, 0) AS INT) AS "likesCnt", COALESCE(likes.liked, FALSE) AS liked,
            users.obj AS "postedBy",
            posts.obj AS "postedTo"
    FROM solutions S
    INNER JOIN(
        SELECT
            users.id,
        ...
        FROM users
        LEFT JOIN occupations ON users.occupation_id = occupations.id
        INNER JOIN industries ON industry_id = industries.id) AS users ON users.id = S.user_id
    INNER JOIN(
        SELECT
            posts.id,
            json_build_object('id', posts.id,
                            'description', posts.description,
                            'created', extract(epoch from posts.created) * 1000,
                            'topic', topics.obj,
                            'location', cities.name || ', ' || ISO_3_code.name) AS obj
        FROM posts
        INNER JOIN cities ON city_id = cities.id
        INNER JOIN states ON states.id = cities.state_id
        INNER JOIN countries ON countries.id = states.country_id
        ...
        INNER JOIN(
            SELECT
                ...
                json_build_object('id', topics.id,
                                'name', INITCAP(topics.name)) AS obj
            FROM topics) AS topics ON topics.id = posts.topic_id) AS posts ON posts.id = S.post_id
    LEFT JOIN(
        SELECT
            solution_id,
            ${firstPersonId}=ANY(array_agg(user_id)) AS liked,
            COUNT(id) AS count
        FROM likes
...
module.exports = { getUserFeed, sameHereUsers, getPendingPosts, getNewNotificationCount, getNotifications, getUserRejectedPost, getSolutions}